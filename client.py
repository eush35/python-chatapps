	...
	while True:
		...
		if msg == "/viewRequests":
			...
			if response == "/proceed":
				...
				print("Lutfen kullanici adinizi onaylayin: ")
			...
			serverSocket.send(bytes(".","utf-8"))
			data = pickle.loads(serverSocket.recv(1024))
			print("Gruptaki Tum Kullanicilar:")
			for element in data:
				print(element)
		elif msg == "/onlineMembers":
			serverSocket.send(bytes(".","utf-8"))
			data = pickle.loads(serverSocket.recv(1024))
			print("Aktif Kullanicilar:")
			for element in data:
				print(element)
			...
			if response == "/proceed":
				state["inputMessage"] = False
				print("Yeni yoneticinin kullanici adini giriniz: ")
				with state["inputCondition"]:
					state["inputCondition"].wait()
				state["inputMessage"] = True
				serverSocket.send(bytes(state["userInput"],"utf-8"))
			...
			if response == "/proceed":
				state["inputMessage"] = False
				print("Oturumdan atmak istediginiz kullanici: ")
				with state["inputCondition"]:
					state["inputCondition"].wait()
				state["inputMessage"] = True
				serverSocket.send(bytes(state["userInput"],"utf-8"))
			...
			state["alive"] = False
			state["inputMessage"] = False
			print("Oturumdan atildiniz, ayrilmak icin herhangi tusa basiniz.")
			break
		elif msg == "/fileTransfer":
			state["inputMessage"] = False
			print("Dosya ismi giriniz: ")
			with state["inputCondition"]:
				state["inputCondition"].wait()
			state["inputMessage"] = True
			...
			print("Gruptan dosya gonderildi...")
			...
			while remaining:
				...
				f.write(data)
			f.close()
			print("Gonderilen dosyayı farkli kaydet",filename)
		else:
			print(msg)
...
def waitServerListen(serverSocket):
	while not state["alive"]:
		...
		if msg == "/accepted":
			state["alive"] = True
			print("Katilma isteginiz onaylandi. Mesaj gonderebilirsiniz.")
			break
...
def main():
	if len(sys.argv) < 3:
		print("Kullanim: python client.py <IP> <Port>")
		return
	...
	state["username"] = input("Redhope sohbet uygulamasına hosgeldiniz, Adiniz-Soyadiniz: ")
	state["groupname"] = input("Baglanmak istediginiz grup: ")
	state["alive"] = False
	...
	if response == "/adminReady":
		print("Yeni grup oluşturdunuz,",state["groupname"],"ve artık yonetici sizsiniz.")
		state["alive"] = True
	elif response == "/ready":
		print("Gruba katildiniz",state["groupname"])
		state["alive"] = True
	elif response == "/wait":
		print("Gruba katilma isteginiz yonetici onayi bekliyor.")
	...
	while True:
		if state["alive"] or state["joinDisconnect"]:
			break
	if state["alive"]:
		print("Kullanilabilir Komutlar:\n/1 -> Katilma Talepleri (Adminler icin)\n/2 -> Katilma Istekleri (Admine)\n/3 -> Baglantiyi kes!\n/4 -> Butun Kullanicilari Gor\n/5 -> Aktif Kullanicilari Gor\n/6 -> Yonetici Degisikligi\n/7 -> Grup Yoneticisi\n/8 -> Uyenin Oturumunu Sonlandir\n/9 -> Dosya Transferi \nHerhangi mesaj gonderin!")
	...
	while True:
		if state["joinDisconnect"]:
			...
			waitUserInputThread.join()
			waitServerListenThread.join()
			print("Redhope oturumdan ayrildiniz.")
			break
